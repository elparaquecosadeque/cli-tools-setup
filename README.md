...
1. Instala 15 paquetes con winget (git, gh, go, node, python 3.14, .NET SDK 8, cloudflared, VS Code, PostgreSQL 16, ffmpeg, gpg, vim, ripgrep, PowerToys, Windows Terminal) + Notepads App (Microsoft Store).
2. Instala las 17 extensiones de VS Code que usás (requiere que `code` ya esté en el PATH desde el paso anterior).
3. Agrega el bin de PostgreSQL al PATH (en la laptop vieja faltaba: `psql` no era accesible).
4. Instala paquetes npm globales: `@angular/cli`, `deno`, + `corepack enable`.
5. Instala paquetes pip: `torch`, `openai-whisper`, `pillow`, `pillow_heif`, `numba` (setup de transcripción con Whisper).
6. Clona `terminal-scripts`, recrea su `.env` (`REPOS_DIRECTORY`) y corre su propio `install.ps1` para sumarlo al PATH.
7. Instala Claude Code (si no está) y copia `claude-config/` a `~/.claude/` (settings con los 4 plugins + status line + comando `/gi`).
8. Imprime los pasos manuales que no se pueden automatizar (login de `gh`, git user/email, llaves SSH/GPG).
